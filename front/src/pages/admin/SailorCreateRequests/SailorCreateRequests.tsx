...
const AdminRequestsPage: React.FC = () => {
  ...
  const fetchRequests = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`http://51.250.27.60/api/admin/sailor-create-requests?${params.toString()}`, {
        headers: {
  ...
  const handleApprove = async (id: string) => {
    if (!token) return;
    try {
      await fetch(`http://51.250.27.60/api/admin/sailor-create-requests/approve/${id}`, {
        method: 'POST',
  ...
  const handleReject = async (id: string) => {
    if (!token) return;
    try {
      await fetch(`http://51.250.27.60/api/admin/sailor-create-requests/reject/${id}`, {
        method: 'POST',
        headers: { Authorization: `Bearer ${token}` },
      });
      fetchRequests();
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      await fetch(`http://51.250.27.60/api/admin/sailor-create-requests/delete/${id}`, {
        method: 'DELETE',
        headers: { Authorization: `Bearer ${token}` },
      });
      fetchRequests();
