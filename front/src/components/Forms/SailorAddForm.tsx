...
const SailorAddForm: React.FC<SailorFormProps> = ({
  ...
  const uploadPhoto = async (file: File) => {
    ...
    try {
      const res = await fetch('http://51.250.27.60/api/upload/image', {
        method: 'POST',
        body: formDataToSend,
      });
  ...
  return (
    <form className="common-form" onSubmit={handleSubmit} noValidate>
      ...
      <PhotoUpload photoUrl={formData.photo_url} uploading={uploadingPhoto} error={errors.photo_url} dragOver={dragOver} onFileChange={handleFileSelected} onDrop={handleDrop} onDragOver={handleDragOver} onDragLeave={handleDragLeave} />
