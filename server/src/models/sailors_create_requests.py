...
class SailorCreateRequest(MyDeclarativeBase, SailorBase):
    ...
    id: Mapped[UUID] = mapped_column(
        ...
        default=uuid4,
        index=True,
        comment="Идентификатор заявки"
    )

    user_fullname: Mapped[str] = mapped_column(
        String(100),
        nullable=False,
        comment="ФИО пользователя, подавшего заявку"
    )
    ...
    additional_information: Mapped[str | None] = mapped_column(
        String(500),
        nullable=True,
        comment="Дополнительные сведения от пользователя, например для связи(телефон, telegram, vk и т.д.)"
    )
