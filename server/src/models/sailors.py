...
class SailorBase:
    ...
    surname: Mapped[str] = mapped_column(
        String(30),
        nullable=False,
        comment="Фамилия моряка"
    )

    patronymic: Mapped[str] = mapped_column(
        String(30),
        nullable=False,
        comment="Отчество моряка"
    )

    birth_date: Mapped[datetime.date] = mapped_column(
        Date,
        nullable=False,
        comment="Дата рождения (ГГГГ-ММ-ДД)"
    )

    death_date: Mapped[datetime.date | None] = mapped_column(
        Date,
        nullable=True,
        comment="Дата смерти (ГГГГ-ММ-ДД), если известна"
    )

    admission: Mapped[int] = mapped_column(
        nullable=False,
        comment="Номер набора в школу"
    )

    photo_url: Mapped[str] = mapped_column(
        String(500),
        nullable=True,
        comment="Ссылка на фото моряка"
    )
    ...
    def validate_admission(self, key, value):
        if value < 1 or value > 3:
            raise ValueError("Номер набора варьируется от 1 до 3 включительно")
        return value
